...
class Validate {
    ...
    public function birthdate($name, $value, $required = ture) {
        $field = $this->fields->getField($name);
        ...
        $message = "Invalid entry; must be in format mm/dd/yyyy.";
        $this->pattern($name, $value, $pattern, $message, $required);
        ...
        if ($birthdate > $curDate){
            $field->setErrorMessage('Birthday must be before'.$curDate);
            return;
        }        
    }
