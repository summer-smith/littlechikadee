...
if(isset($_SESSION['compound_monthly'])) {
    $compound_monthly = $_SESSION['compound_monthly'];
}

$action = filter_input(INPUT_POST, 'action');

//Calculate future value
function calculateFutureValue($investment, $years, $interest_rate, $compoundMonthly = FALSE){
        if($compoundMonthly){
        $is_monthly_f = "Yes";
        ...
        $time = $years *  12;
        for ($i = 1; $i <= $time; $i++) {
            $futureValue = $futureValue + ($futureValue * (($interest_rate * .01)/12));
        }
    }else {
        $is_monthly_f = "No";
        // calculate the future value
...
if ($action == 'display_form'){
    ...
    if (!isset($compound_monthly)) {$compound_monthly = FALSE;}
    ...
    $future_value = calculateFutureValue($investment, $years, $interest_rate, $compound_monthly);
